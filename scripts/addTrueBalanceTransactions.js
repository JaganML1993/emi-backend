...
// Configuration
const EMI_ID = '68aaefe899f1a02e5565c481';
const USER_ID = '68a9fa309f4cc06312f240b9';
const MONTHLY_AMOUNT = 3979;
const START_DATE = '2025-06-03';

async function addTrueBalanceTransactions() {
  try {
    // Connect to database
    ...
    // Calculate installments from June 3, 2025 to current month
    const startDate = new Date(START_DATE);
    const currentDate = new Date();
    
    // Generate monthly dates from June 3, 2025
    const installments = [];
    ...
    const existingTransactions = await Transaction.find({
      user: USER_ID,
      description: { $regex: /True Balance.*installment/i },
    ...
    for (let i = 0; i < installments.length; i++) {
      ...
      const existingTx = await Transaction.findOne({
        user: USER_ID,
        description: { $regex: new RegExp(`True Balance.*installment.*${installmentNumber}`, 'i') },
      ...
      const transaction = await Transaction.create({
        ...
        description: `True Balance - Installment ${installmentNumber}`,
