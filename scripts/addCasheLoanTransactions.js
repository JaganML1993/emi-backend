...
// Configuration
const EMI_ID = '68aaed4fa96978420ad4494d';
const USER_ID = '68a9fa309f4cc06312f240b9';
const MONTHLY_AMOUNT = 10403;
const START_DATE = '2025-05-01';

async function addCasheLoanTransactions() {
  try {
    // Connect to database
    ...
    // Calculate installments from May 1, 2025 to current month
    const startDate = new Date(START_DATE);
    const currentDate = new Date();
    
    // Generate monthly dates from May 1, 2025
    const installments = [];
    ...
    const existingTransactions = await Transaction.find({
      user: USER_ID,
      description: { $regex: /Cashe.*installment/i },
    ...
    for (let i = 0; i < installments.length; i++) {
      ...
      const existingTx = await Transaction.findOne({
        user: USER_ID,
        description: { $regex: new RegExp(`Cashe.*installment.*${installmentNumber}`, 'i') },
      ...
      const transaction = await Transaction.create({
        ...
        description: `Cashe - Installment ${installmentNumber}`,
